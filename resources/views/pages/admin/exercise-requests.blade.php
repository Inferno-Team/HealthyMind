...
@section('content')
    @include('pages.admin.topnav', ['title' => 'Exercise Requests'])
                                ...
                                <tbody id="requests-table">
                                    @foreach ($requests as $request)
                                        <tr id="item-{{ $request->id }}">
                                            ...
                                            <td>
                                                <p class="text-xs font-weight-bold mb-0">{{ $request->equipment?->name }}
                                                </p>
                                            </td>
                                            <td>
                                                @if (!empty($request->media))
                                                    <p class="text-xs font-weight-bolder mb-0 show-media"
                                                        style="cursor: pointer;" data-url="{{ $request->media }}">
                                                        show
                                                    </p>
                                                @else
                                                    <p class="text-xs font-weight-bold mb-0">
...
@section('js-script')
    <script>
        ...
        function sendChangeStatus(id, status) {
            ...
            axios({
                    method: 'POST',
                    url: "{{ route('admin.exercise.request.change-status') }}",
                    data: {
                        id: id,
                        status: status
                    }
