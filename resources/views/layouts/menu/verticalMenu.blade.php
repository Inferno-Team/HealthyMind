    ...
    <ul class="menu-inner py-1">
        @foreach ($menuData[0]->menu as $menu)
            @if ($menu->dialog ?? false)
                <li class="menu-item {{ $menu->dialogOpenerID }}"
                    qr-code-link="{{ asset('/storage/payment_qr/QR.jpg') }}"
                    upload-url="{{ route('admin.qr.update') }}"
                    ...
                    {{-- submenu --}}
                    @isset($menu->submenu)
                        @include('layouts.menu.submenu', ['menu' => $menu->submenu])
                    @endisset
                </li>
            @else
